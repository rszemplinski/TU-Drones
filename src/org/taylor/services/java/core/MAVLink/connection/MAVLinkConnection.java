...
public abstract class MavLinkConnection {
    ...
    */
    ...
    private final ConcurrentHashMap<String, Pair<String, BufferedOutputStream>> loggingOutStreams = new
            ConcurrentHashMap<String, Pair<String, BufferedOutputStream>>();
     ...
     */
    private final LinkedBlockingQueue<byte[]> mPacketsToSend = new LinkedBlockingQueue<byte []>();

    /**
     * Queue the set of packets to log. A thread will be blocking on it until
     * there's element(s) available for logging.
     */
    private final LinkedBlockingQueue<byte[]> mPacketsToLog = new LinkedBlockingQueue<byte []>();
            ...
            try {
                while (mConnectionStatus.get() == MAVLINK_CONNECTED) {
                    ...
                    for (Map.Entry<String, Pair<String, BufferedOutputStream>> entry : loggingOutStreams
                        ...
                        final String loggingFilePath = logInfo.getKey();
                        try {
                            BufferedOutputStream logWriter = logInfo.getValue();
                            if (logWriter == null) {
                                logWriter = new BufferedOutputStream(new FileOutputStream(loggingFilePath));
                                loggingOutStreams.put(entry.getKey(), new Pair<String, BufferedOutputStream>(loggingFilePath, logWriter));
                            }
                ...
                for (Pair<String, BufferedOutputStream> entry : loggingOutStreams.values()) {
                    final String loggingFilePath = entry.getKey();
                    try {
    ...
    public void addLoggingPath(String tag, String loggingPath) {
        ...
        if (!loggingOutStreams.contains(tag)) {
            loggingOutStreams.put(tag, new Pair<String, BufferedOutputStream>(loggingPath, null));
        }
    }
    ...
    public void removeLoggingPath(String tag) {
        ...
        if (logInfo != null) {
            BufferedOutputStream outStream = logInfo.getValue();
            if (outStream != null) {
                try {
                    outStream.close();
                } catch (IOException e) {
                    mLogger.logErr(TAG, "IO Exception while closing " + logInfo.getKey(), e);
                }
            }
        }
    }
    ...
    public void addMavLinkConnectionListener(String tag, MavLinkConnectionListener listener) {
        ...
        if (getConnectionStatus() == MAVLINK_CONNECTED) {
            ...
            extras.put(LinkConnectionStatus.EXTRA_CONNECTION_TIME, mConnectionTime.get());
    ...
    protected void reportConnect(long connectionTime) {
        HashMap<String, Object> extras = new HashMap<String, Object>();
        extras.put(LinkConnectionStatus.EXTRA_CONNECTION_TIME, connectionTime);
